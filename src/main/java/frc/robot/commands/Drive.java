...
public class Drive extends Command {
  ...
  public void execute() {
    ...
    // else do nothing
    swerveDrivetrain.drive(new Translation2d(getTranslation(), getStrafe()).times(swerveDrivetrain.governor.getGovernor()*Constants.RobotConstants.driveMaxVelo), getRotation()*(Constants.RobotConstants.rotationMaxAngleVelo*swerveDrivetrain.governor.getGovernor())*.5);
  ...
  private double getStrafe() {
    //return 0;
    return MathUtil.applyDeadband(strafeSupplier.getAsDouble(), 0.1);
   // return Math.abs(strafeSupplier.getAsDouble()) < 0.15 ? 0 : (strafeSupplier.getAsDouble()*governor);
  }

  private double getTranslation() {
    //return 0;
    return MathUtil.applyDeadband(translationSupplier.getAsDouble(), 0.1);
